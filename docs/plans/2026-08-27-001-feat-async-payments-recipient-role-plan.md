...
- Q1 **Answered 2026-08-27: the LSP runs the invoice-server role.** U2 onward proceeded on that basis. The serialization half is still open — this plan assumes hex of `BlindedMessagePath.write()`, which the harness confirms round-trips through `constructor_read`, but the operator has not confirmed that is the export format they offer.
