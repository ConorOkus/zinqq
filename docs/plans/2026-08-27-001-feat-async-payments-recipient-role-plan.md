...
- KTD5. **`enable_htlc_hold` is `true`** — corrected 2026-08-28. It was initially left `false` on the reading that it is purely the always-online node's flag. That reading covered only half of it: the flag also gates the `htlc_hold` bit in our advertised init and node features, verified against real LDK (false → not advertised, true → advertised in both). Leaving it false is why the LSP reported `HtlcHold: not supported` for every connected peer. LDK's "only for nodes which expect to be online reliably" warning applies to the holding half, which is inert here — holding applies to HTLCs we forward, and this is a leaf node with one LSP channel that never forwards.
...
- **Single-recipient constraint — a correctness gate, not a roadmap item.** The server keys its static-invoice store on the `recipient_id` baked into the paths (`static_invoices/<sha256(recipient_id)>/<invoice_slot>`), so two wallets sharing a bundle overwrite each other's invoices in a shared keyspace, silently. Nothing at runtime can detect it, so `src/ldk/config.ts` refuses to start unless the build explicitly acknowledges single-recipient operation.
