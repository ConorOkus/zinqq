...
- KTD1. **Server paths arrive as build-time configuration.** Valid only for a single-recipient deployment: `blinded_paths_for_async_recipient` documents that the `recipient_id` it takes "must uniquely identify the recipient," and that id is carried inside the issued paths, so one path set names one wallet. Rejected: negotiating paths over an LSPS extension, which would require a protocol that does not exist today and is the real prerequisite for multi-user support.
  ...
  **The encoding is unresolved — see Q1.** The shipped parser reads hex of `BlindedMessagePath.write()`. That is LDK's internal `Writeable` encoding, not a BOLT wire format: no other implementation parses it, and it is pinned to an LDK revision. The wallet is on `lightningdevkit` 0.2.4-0 (LDK v0.2.4-35-g7b76df2aeae61b3e) and the server is on `lightning` 0.3.0+git — different revisions, so the harness's round-trip proof covers same-version only and says nothing about interop between these two nodes. Do not treat the current parser as interoperable with the LSP until Q1 closes.
