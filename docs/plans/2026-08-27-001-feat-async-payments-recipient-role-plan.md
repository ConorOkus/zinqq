...
- Q1 (blocking for U2 onward, not for U1). Does the wallet's LSP run the static invoice server role, publish per-recipient blinded paths, and in what serialization? External research found async-payments server support in LDK Node 0.7.0 and LDK Server, but no public confirmation for this wallet's LSP. Answering this is a cheap conversation with the operator and gates the value of everything after U1.
...
- KTD1. **Server paths arrive as build-time configuration**, hex-encoded in `VITE_`-prefixed variables and decoded with `BlindedMessagePath.constructor_read`. This is valid only for a single-recipient deployment: `blinded_paths_for_async_recipient` documents that the `recipient_id` it takes "must uniquely identify the recipient," and that id is carried inside the issued paths, so one path set names one wallet. A bundle-baked variable therefore cannot serve multiple users. Rejected: negotiating paths over an LSPS extension, which would require a protocol that does not exist today and is the real prerequisite for multi-user support.
...
- KTD6. **Verification is an in-process handshake harness plus a manual runbook.** The bindings expose the server side (`blinded_paths_for_async_recipient`, `respond_to_static_invoice_request`), so a second `ChannelManager` acting as server makes the handshake executable without a live counterparty. The runbook covers what only a real server can prove.
