...
- [`docs/solutions/infrastructure/vercel-staging-vss-serverless-proxy.md`](../infrastructure/vercel-staging-vss-serverless-proxy.md) — Production half of the same VSS pattern. Documents the Vercel serverless proxy approach for a CORS-less upstream and the `vercel.json` SPA-rewrite negative-lookahead (`/((?!api/).*)`) gotcha that any new `/api/*` proxy must respect.
