...
describe('persistChannelManager', () => {
  ...
  it('tracks version across multiple writes', async () => {
    ...
    expect(vssClient.putObject).toHaveBeenNthCalledWith(2, 'channel_manager', expect.any(Uint8Array), 1)
  })
  ...
  it('throws on VSS failure (caller handles retry)', async () => {
    ...
    await expect(persistChannelManager(cm, { vssClient, cmVersionRef }))
  ...
  it('resolves version conflict by re-fetching server version', async () => {
    ...
    expect(vssClient.putObject).toHaveBeenNthCalledWith(2, 'channel_manager', expect.any(Uint8Array), 5)
  ...
  it('resets version to 0 when getObject returns null during conflict', async () => {
    ...
    expect(vssClient.putObject).toHaveBeenNthCalledWith(2, 'channel_manager', expect.any(Uint8Array), 0)
    expect(cmVersionRef.current).toBe(1)
  })
